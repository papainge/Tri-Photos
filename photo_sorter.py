...
# Conteneurs vidéo dont la date de création embarquée se lit sans dépendance externe.
# MPG/MPEG n'a pas d'équivalent standardisé et reste toujours daté via la date de
...
def get_media_date(path: Path) -> datetime:
    ...
    Tous les formats photo gérés par l'application passent par Pillow, qui lit l'EXIF
    quand il est présent (silencieusement absent sinon, ex: GIF, BMP). Côté vidéo,
    MP4/MOV/M4V/3GP, AVI, WMV et MKV/WEBM ont chacun leur propre parseur sans
    dépendance externe ; seul MPG/MPEG n'a pas d'équivalent standardisé et reste
    toujours daté via la date de modification.
    """
    ...
    if suffix in IMAGE_EXTENSIONS:
        ...
        try:
            ...
            pass
    elif suffix in RIFF_VIDEO_EXTENSIONS:
        try:
            date = get_avi_creation_date(path)
            if date is not None:
                return date
        except Exception:
            pass
    elif suffix in ASF_VIDEO_EXTENSIONS:
        try:
            date = get_wmv_creation_date(path)
            if date is not None:
                return date
        except Exception:
            pass
    elif suffix in MATROSKA_VIDEO_EXTENSIONS:
        try:
            date = get_matroska_creation_date(path)
            if date is not None:
                return date
        except Exception:
            pass

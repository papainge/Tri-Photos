...
1. Cliquer sur **Ajouter...** sous **Dossiers sources** pour sélectionner un dossier
   contenant les photos et vidéos à trier ; répéter pour ajouter d'autres dossiers
   sources (cartes SD de plusieurs appareils, exports de plusieurs téléphones...) à
   trier en une seule fois, comme s'ils n'en formaient qu'un seul (**Retirer** enlève le
   dossier sélectionné dans la liste). Un nombre total de fichiers photo/vidéo trouvés
   dans les dossiers sélectionnés s'affiche alors immédiatement (comptage rapide, sans
   lecture des dates) — un ordre de grandeur avant de lancer l'analyse complète,
   potentiellement plus longue. La case **Inclure les sous-dossiers** (cochée par
   défaut) détermine si l'analyse descend dans les sous-dossiers ou se limite à la
   racine de chaque dossier choisi ; la décocher met aussi à jour ce comptage rapide
   immédiatement. Contrairement au niveau de tri et à la séparation Photos/Vidéos (voir
   ci-dessous), ce réglage ne peut pas se contenter de réorganiser l'arborescence déjà
   analysée : la changer après une analyse affiche un rappel invitant à recliquer sur
   **Analyser** pour l'appliquer.
...
7. Cliquer sur **Choisir...** (destination) pour désigner le dossier dans lequel créer
   l'arborescence. Ce dossier ne peut être aucun des dossiers sources analysés, ni un de
   leurs sous-dossiers (refusé au moment de lancer la copie, pour éviter de copier les
   fichiers dans un dossier en cours d'analyse).

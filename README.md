...
2. Pour trier en une seule fois plusieurs cartes SD ou exports de téléphones différents,
   ajouter chacun via **Ajouter...** sous **Dossiers sources supplémentaires
   (optionnel)** ; **Retirer** enlève le dossier sélectionné dans la liste. Le dossier
   source principal (étape précédente) est optionnel dès qu'au moins un dossier
   supplémentaire est choisi. Tous les dossiers sources sont analysés et copiés ensemble,
   comme s'ils n'en formaient qu'un seul.
3. Cocher **À défaut, essayer de détecter une date dans le nom du fichier** si vous
...
4. Choisir le **niveau de tri** souhaité : *Année*, *Année / Mois* ou
...
5. Cocher **Séparer Photos et Vidéos à la racine de la destination** si vous voulez deux
...
6. Cocher **Renommer les fichiers selon la date (AAAA-MM-JJ_HHMMSS)** si vous voulez
...
7. Cliquer sur **Analyser** : l'arborescence correspondant aux choix ci-dessus, avec le
   ...
   à la volée.
8. Cliquer sur **Choisir...** (destination) pour désigner le dossier dans lequel créer
   l'arborescence. Ce dossier ne peut être aucun des dossiers sources analysés (principal
   ou supplémentaires), ni un de leurs sous-dossiers (refusé au moment de lancer la
   copie, pour éviter de copier les fichiers dans un dossier en cours d'analyse).
9. Choisir l'**action** : *Copier* (les originaux sont conservés, par défaut) ou
   *Déplacer* (les originaux sont supprimés une fois transférés).
10. Cliquer sur **Créer l'arborescence et copier/déplacer les fichiers**. Une copie (ou
